...
Status: Phase 0 observability foundation complete; Phase 1 durable conversation runtime complete as of 2026-07-26; Phase 2 (workspace context and knowledge layer) is next  
...
- **Not yet wired**: `SmartField` is not mounted on any form. It needs one integration point to become real — the proposal or contract scope field is the natural first — and that wiring is deliberately left until the app has been exercised, since it touches an existing form's state management.
...
- Known gap outside this slice: `ai-workflows/actions.ts` still exposes `generateInvoiceDraftAction` and `generateOperationalDraftAction` as public endpoints. They generate rather than mutate, so they are lower risk, but they consume model quota without passing the runtime's rate and quota policy.
