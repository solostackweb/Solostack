...
Current slice: workspace-native `@` mentions now bind prompts to ownership-checked clients, projects, invoices, and welcome documents. Selected records are reread server-side and injected as explicitly untrusted data, so displayed labels never act as authority and stale or foreign ids are discarded.
