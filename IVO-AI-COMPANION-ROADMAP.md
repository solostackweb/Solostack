...
Current slice: questionnaire drafts now stay open for conversational refinement. A follow-up such as “make question 3 optional” or “add a budget question” produces a complete before/after proposal without mutating canonical data. The user must explicitly apply the exact hash-bound proposal; the tool then rejects stale originals, updates only the owned internal draft, and leaves client delivery as a separate action.
