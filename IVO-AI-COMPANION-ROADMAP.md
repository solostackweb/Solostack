...
Current slice: meeting schedule requests now read and display the user's canonical workspace meetings instead of falling through to help documentation. “Check my meetings” deterministically opens a grounded upcoming/awaiting list before Groq can reinterpret it. Cards show topic, client, duration, human status, and the scheduled time in the meeting timezone, with Open, Copy booking link, and Join call actions where applicable.
