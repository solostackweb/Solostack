...
Current slice: IVo and the shared meeting action now refuse to create or email a live-availability booking link unless it has at least one genuinely bookable time in the next 14 days. IVo checks Google Calendar readiness and the effective working-hours/busy-time result before presenting confirmation, then offers direct paths to Availability settings or the manual specific-times scheduler. The shared mutation repeats the slot check immediately before insertion, protecting manual callers and calendar changes between preview and approval.
