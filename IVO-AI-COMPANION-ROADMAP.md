...
Current slice: conversational client/project reminders now produce a real, durable email approval card instead of a copy-only prose draft. IVo resolves the named owned client and relevant active project, validates their relationship and current recipient email, drafts through Groq with a grounded deterministic fallback, and displays recipient, subject, body, Copy, Open project, and explicit Approve & send controls. The exact card survives conversation reload, and a typed “yes, send this” acts on its bound prepared-action ID through the existing audited delivery tool.
