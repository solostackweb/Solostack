...
Current slice: portal creation is now a first-class IVo workflow instead of being answered as a generic checklist or misrouted into a project named “Client Portal.” IVo asks for the client, reads the canonical live contracts, issued invoices, and published welcome documents that can be shared, previews the portal name and invitation recipient, requires explicit approval, then creates the real portal and emails its invitation through the audited portal tool.
...
- Phase 1 slice 33: meeting reads now recognize natural first-person schedule questions in addition to imperative list commands. The exact follow-up chip emitted after meeting creation is covered end-to-end by the deterministic planner, so “What meetings do I have coming up?” can no longer route into documentation or support. Equivalent upcoming-call, next-meeting, and personal-calendar questions share the canonical owner-scoped read, while how-to questions remain excluded to prevent false positives.
- Phase 1 slice 34: live meeting links are availability-gated before delivery. IVo mirrors the manual scheduler's Google Calendar connection requirement, computes the actual next 14 days of working hours minus busy periods before showing approval, and refuses to email a dead booking page. The canonical create action repeats that computation directly before inserting any availability-mode meeting, closing race conditions and protecting every caller. When setup is incomplete or the calendar is fully blocked, IVo presents buttons for Availability settings and the prefilled manual specific-times flow.
- Phase 1 slice 35: client cards are now direct workflow launchers rather than oversized two-action rows. Contract and Meeting join Open and Invoice on every card, with both the live list and canonically restored conversation renderer wiring the selected client name into IVo's guided creation flow. The responsive wrapping layout preserves the full-width data-card treatment on narrow companion panels.
- Phase 1 slice 36: client and project picker loading is now race-safe. A single cached promise warms on panel open and is explicitly awaited whenever the server's next-action descriptor requests a picker, covering entry-point prompts that open and submit in the same event. The picker is built from the resolved snapshot—not the render's stale empty state—so later state updates are not required to repair a message whose React element has already been stored. Load failures clear the promise for retry and never masquerade as an empty workspace.
- Phase 1 slice 37: document list cards now share one status presentation grammar across invoices, contracts, and proposals. Drafts are visually muted and explicitly marked “Not sent”; sent/viewed records use the live primary treatment; successful terminal states use green; overdue/declined/expired use destructive emphasis; partial payment uses amber; and cancelled records are subdued. Invoice rows gain the same audited Send/Resend action already used by contracts and proposals, while reminder and payment actions appear only after first delivery. Both fresh and restored invoice lists use the delivery handler.
