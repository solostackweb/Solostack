...
export function buildSystemPrompt(input: IvoAgentPromptInput, memories: string[]): string {
  ...
  return [
    ...
    Object.keys(input.collected).length > 0
      ...
      : "",
    input.pendingField
      ? `- ⚠️ FIELD ANSWER MODE. The UI is waiting for ONE specific field: "${input.pendingField.field}"${input.currentMode !== "general" ? ` of the current ${input.currentMode} task` : ""}. The user's message IS the answer to that field — take it literally as the value, EVEN IF it contains words like "create", "creating", "new", "invoice", "project", "home page", or a client/project name. Do NOT call start_task for a different task, do NOT change the task type, do NOT re-ask anything already answered. Continue the SAME task via start_task, passing this field PLUS everything in "Details collected so far". If the field is OPTIONAL and the user replies "skip" (or "none", "no thanks", "later"), pass "__skip__" as that field's value and continue the SAME task — that is a skip, never a cancellation. The ONLY exceptions: the user clearly cancels ("stop", "cancel", "never mind") OR clearly corrects the task itself ("no, I meant an invoice, not a project") — only then change course.`
      : "",
    input.activeDraft
