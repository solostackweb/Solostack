  ...
  const tools: GroqToolDefinition[] = [
    ...
    // ---- ROUTE tools (terminate the loop) --------------------------------
    {
      ...
      function: {
        name: "start_task",
        description:
          "Start (or continue) a guided creation task: draft an invoice, contract/proposal/NDA/retainer, welcome document, create a client, project, or time entry, or schedule a meeting/call (task='meeting'). Pass every detail the user already gave in `fields` so they are never asked twice. The UI then walks the user through anything missing with pickers. For proposals use task='contract' with fields.type='proposal'.",
        parameters: {
          type: "object",
          properties: {
...
function buildSystemPrompt(input: IvoAgentInput, memories: string[]): string {
  ...
  return [
    ...
    "- Creation requests (invoice / contract / proposal / NDA / retainer / welcome doc / client / project / time entry / meeting or call) → call start_task IMMEDIATELY, even if the user gave no details yet — pass whatever they DID provide in fields and leave the rest out. NEVER ask for the client, scope, amount, or any other detail in a plain-text reply: after start_task the UI shows a client picker dropdown and asks each remaining question one at a time with suggestions. Asking in text instead of calling start_task is a mistake. Example: 'help me generate a proposal' → start_task {task:'contract', fields:{type:'proposal'}, reply:'Starting your proposal.'}. A 'proposal' is task='contract' with fields.type='proposal' — call it a proposal in your reply, never a contract.",
