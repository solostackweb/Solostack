  ...
  const tools: GroqToolDefinition[] = [
    ...
    },
    {
      ...
      function: {
        name: "remember",
        description:
          "Save ONE durable preference or fact to long-term memory so future conversations honour it — e.g. 'Standard rate is ₹2,500/hr', 'Always Net-15 for Kumar Associates', 'Sign emails as Arpit from Developer Bazaar'. Use ONLY when the user states a lasting preference or explicitly says to remember something. Never store transient details, secrets, or anything about other people's private data.",
        parameters: {
          type: "object",
          properties: {
    ...
    // ---- ROUTE tools (terminate the loop) --------------------------------
    {
      ...
      function: {
        name: "start_task",
        description:
          "Start (or continue) a guided creation task: draft an invoice, contract/proposal/NDA/retainer, welcome document, or create a client, project, or time entry. Pass every detail the user already gave in `fields` so they are never asked twice. The UI then walks the user through anything missing with pickers. For proposals use task='contract' with fields.type='proposal'.",
        parameters: {
          type: "object",
          properties: {
...
function buildSystemPrompt(input: IvoAgentInput, memories: string[]): string {
  ...
  return [
    ...
    Object.keys(input.collected).length > 0
      ...
      : "",
    input.pendingField
      ? `- The UI is currently asking the user for: ${input.pendingField.field}. If their message answers it, continue via start_task with that field included.`
      : "",
    input.activeDraft
    ...
    "",
    memories.length > 0
      ? `REMEMBERED PREFERENCES — apply these automatically without being asked:\n${memories
          .map((memory) => `- ${memory}`)
          .join("\n")}`
      : "",
