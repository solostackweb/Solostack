...
export function buildSummaryCsv(
  ...
  const rows: Array<[string, string]> = [
    ...
    ["Proposals sent", String(insights.proposals.sent)],
    ["Proposals accepted", String(insights.proposals.accepted)],
    ["Proposal win rate %", insights.proposals.winRatePct === null ? "" : fmt(insights.proposals.winRatePct)],
    ["Proposal accepted value", fmt(insights.proposals.acceptedValue)],
    ["Domestic paid revenue", fmt(insights.global.domesticPaid)],
    ["International paid revenue", fmt(insights.global.internationalPaid)],
    ["International revenue share %", insights.global.internationalPct === null ? "" : fmt(insights.global.internationalPct)],
    ["Cash forecast overdue", fmt(insights.cashForecast.overdue)],
    ["Cash forecast due 7 days", fmt(insights.cashForecast.due7Days)],
    ["Cash forecast due 30 days", fmt(insights.cashForecast.due30Days)],
    ["Cash forecast projected 30 days", fmt(insights.cashForecast.projected30Days)],
