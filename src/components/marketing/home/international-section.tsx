            ...
            >
              <ul className="mt-3 flex flex-wrap gap-2">
                {CURRENCIES.map((c) => (
                  <li
                    key={c}
                    className="rounded-lg border border-border/70 bg-background px-2.5 py-1 font-mono text-[12px] font-medium tabular-nums text-foreground"
                  >
                    {c}
                  </li>
                ))}
                <li className="rounded-lg border border-dashed border-border/70 px-2.5 py-1 text-[12px] font-medium text-muted-foreground">
                  +20 more
                </li>
              </ul>
            </ProofRow>
            ...
            >
              <ul className="mt-3 flex flex-wrap gap-2">
                {PLATFORMS.map((p) => (
                  <li
                    key={p.name}
                    className="flex items-center gap-1.5 rounded-lg border border-border/70 bg-background px-2.5 py-1 text-[12px] font-medium text-foreground"
                  >
                    <span
                      aria-hidden
                ...
                <li className="flex items-center gap-1.5 rounded-lg border border-border/70 bg-background px-2.5 py-1 text-[12px] font-medium text-foreground">
      ...
      <div className="overflow-hidden rounded-2xl border border-border/80 bg-card shadow-2xl shadow-primary/[0.10] ring-1 ring-foreground/[0.04]">
        ...
        <div className="flex items-start justify-between gap-4 px-5 py-4">
          <div>
            ...
            <p className="text-[12px] text-muted-foreground">San Francisco, USA</p>
          </div>
          <span className="rounded-lg border border-border/70 bg-background px-2.5 py-1 font-mono text-[12px] font-medium text-foreground">
            USD $
          </span>
        </div>

        {/* Line item */}
        <div className="mx-5 rounded-xl border border-border/60 bg-background/60 p-3.5">
        ...
        <div className="border-t border-border/70 bg-muted/30 px-5 py-3.5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <span className="flex items-center justify-center gap-1.5 rounded-lg bg-foreground px-3 py-2 text-[12px] font-semibold text-background">
              Pay with Wise <ArrowRight className="h-3.5 w-3.5" />
            </span>
            <span className="flex items-center justify-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-[12px] font-semibold text-foreground">
              PayPal
            </span>
          </div>
        </div>
  ...
  return (
    <div className="flex gap-4">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-[14px] leading-relaxed text-muted-foreground">{copy}</p>
        {children}
      </div>
    </div>
  );
...
function ExplainerCard({
  ...
  return (
    <div className="group h-full rounded-2xl border border-border/70 bg-card p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/[0.06] sm:p-6">
      <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-primary/15 to-primary/5 text-primary ring-1 ring-primary/10">
        <Icon className="h-5 w-5" />
      </span>
